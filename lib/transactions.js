...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  if (transaction.is_import) {
      transformed.vout[0].scriptPubKey.addresses = 'import';
      console.log(transformed.vout[0].scriptPubKey);
  }
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  if (output.address) {
    ...
    transformed.scriptPubKey.type = output.scriptType;
  }
  return transformed;
};
